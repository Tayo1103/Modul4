...
<div class="container mx-auto p-6">
    ...
    <div class="bg-white p-8 rounded shadow-lg">
        @if($article->image)
            <img src="{{ asset('storage/' . $article->image) }}" alt="{{ $article->title }}" class="w-full h-96 object-cover rounded mb-4">
        @endif

        <div class="flex justify-between items-center mb-4">
            <h1 class="text-3xl font-bold text-gray-800">{{ $article->title }}</h1>
            <p class="text-sm text-gray-500">{{ $article->created_at->timezone('Asia/Jakarta')->format('d M Y, H:i') }}</p>
        </div>
        <p class="mt-4 text-gray-700 text-justify">{{ $article->content }}</p>
    </div>
    ...
    <div class="mt-8">
        ...
        @empty
            <p class="text-gray-500">There are no comments available.</p>
        @endforelse
    </div>
</div>
@endsection
